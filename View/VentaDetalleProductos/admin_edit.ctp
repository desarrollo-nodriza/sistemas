										...
										<? foreach ($this->request->data['VentaDetalle'] as $ivd => $vd) : ?>
											...
											<? foreach ($vd['VentaDetallesReserva'] as $VentaDetallesReserva) : ?>
												<tr>
													...
													<td><?= $vd['cantidad']; ?></td>
													<td><?= $VentaDetallesReserva['cantidad_reservada']; ?></td>
													<td><?= $VentaDetallesReserva['Bodega']['nombre']; ?></td>
													<td>
														<div class="btn-group">
															...
															<? if ($VentaDetallesReserva['cantidad_reservada'] > 0) : ?>
																<?= $this->Html->link('<i class="fa fa-ban"></i> Liberar', array('controller' => 'ventas', 'action' => 'liberar_stock_reservado', $VentaDetallesReserva['id'],  $vd['venta_id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => 'Liberar stock')) ?>
															<? endif; ?>
														</div>
													</td>
												</tr>
						...
						<div class="panel-body">
							<div class="table-responsive" style="max-height: 470px;">
								<table class="table table-bordered">
									...
									<tbody>
										<? foreach ($this->request->data['VentaDetalle'] as $ivd => $vd) : if (array_sum(Hash::extract($vd['VentaDetallesReserva'], "{n}.cantidad_reservada")) < 1) {
												continue;
											} ?>
												<tr>
													<td><?= $this->Html->link($vd['venta_id'], array('controller' => 'ventas', 'action' => 'view', $vd['venta_id']), array('target' => '_blank')); ?></td>
													...
													<td><?= $this->Html->link('<i class="fa fa-ban"></i> Liberar', array('controller' => 'ventas', 'action' => 'liberar_stock_reservado', 0, $vd['venta_id'], $vd['id'], $vd['venta_detalle_producto_id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle' => 'tooltip', 'title' => 'Liberar stock')) ?></td>
												</tr>
										<? endforeach; ?>
									</tbody>
								</table>
