...
<div class="page-content-wrap">
	...
	<div class="row">
		<div class="col-xs-12 col-md-6 col-md-offset-3">
			...
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="fa fa-cogs" aria-hidden="true"></i> Cargar archivos</h3>
				</div>
				<div class="panel-body">
					<div class="form-group col-xs-12">
						<label>Descarga el archivo de ejemplo</label>
						<br>
						<?=$this->Html->link('<i class="fa fa-download"></i> Ejemplo', '/ejemplos/ejemplo-ajustar-inventario.csv', array('escape' => false, 'target' => '_blank', 'fullbase' => true, 'class' => 'btn btn-xs btn-info')); ?>
					</div>
					<div class="form-group col-xs-12 col-md-6">
						<label>Archivo CSV,XLS,XLSX</label>
						<?=$this->Form->input('archivo', array(
							'class' => '<?=(empty($this->Session->read("ajustarInventarioMasivo"))) ? "not-blank" : "" ; ?>',
							'type' => 'file'
							))?>
					</div>
				</div>
		...
		<div class="col-xs-12">
			<? if (!empty($this->Session->read('ajustarInventarioMasivo.options'))) : ?>
			<div class="panel panel-primary">
				<div class="panel-heading">
				</div>
				<div class="panel-body">
					...
					<div class="table-responsive" style="max-height: 350px; overflow-y: auto;">
						<table class="table table-bordered" >
							...
							<thead>
								<? foreach ($this->Session->read('ajustarInventarioMasivo.options') as $i => $cabecera) : ?>
								<th>
							...
							<tbody class="js-body">
							<? foreach ($this->Session->read('ajustarInventarioMasivo.data') as $id => $valor) : if ($id == 1) continue; ?>
							<tr>
								<? foreach ($this->Session->read('ajustarInventarioMasivo.options') as $i => $op) : ?>
