							...
							<table class="table table-bordered js-clone-wrapper">
								...
								<? foreach ($this->request->data['VentaDetalleProducto'] as $ipp => $pp) : ?>	
									<tr>
										...
										<td><?= $this->Form->input(sprintf('VentaDetalleProducto.%d.precio_unitario', $ipp), array('type' => 'text', 'class' => 'form-control not-blank js-precio-producto', 'value' => $pp['precio_costo'], 'readonly' => 'readonly')); ?></td>
										
										
										<td class="js-descuento-valor">
											<?= $this->Form->input(sprintf('VentaDetalleProducto.%d.descuento_producto', $ipp), array('type' => 'text', 'class' => 'form-control not-blank is-number js-descuento-producto', 'value' => $pp['total_descuento'], 'readonly' => 'readonly', 'data-descuento' => $pp['total_descuento'])); ?>
										</td>
										
										<td>
											<?= $this->Form->input(sprintf('VentaDetalleProducto.%d.total_neto', $ipp), array('type' => 'text', 'class' => 'form-control not-blank is-number js-total-producto', 'value' => ($pp['precio_costo'] - $pp['total_descuento'])*$pp['OrdenComprasVentaDetalleProducto']['cantidad'], 'readonly' => 'readonly')); ?>
										</td>
										<td valign="center">
											<button class="remove_tr btn-danger"><i class="fa fa-minus"></i></button>
										</td>
