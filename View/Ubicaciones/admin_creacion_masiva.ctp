<div class="page-title">
	<h2><span class="fa fa-cube"></span> Creación masiva</h2>
</div>

<?= $this->Form->create('Ubicacion', array('inputDefaults' => array('div' => false, 'label' => false), 'class' => 'js-formulario', 'type' => 'file')); ?>
...
<div class="page-content-wrap">
	...
	<div class="row">
			...
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<div class="form-group col-xs-12">
						...
						<br>
						<?=$this->Html->link('<i class="fa fa-download"></i> Ejemplo', '/ejemplos/ejemplo-creacion-masiva-zonas.xlsx', array('escape' => false, 'target' => '_blank', 'fullbase' => true, 'class' => 'btn btn-xs btn-info')); ?>
					</div>
					<div class="form-group col-xs-12 col-md-6">
						<label>Archivo CSV,XLS,XLSX</label>
						<?=$this->Form->input('archivo', array(
							'class' => '<?=(empty($this->Session->read("creacionMasivaUbicaciones"))) ? "not-blank" : "" ; ?>',
							'type' => 'file'
							))?>
					</div>
				</div>
		...
		<div class="col-xs-12">
			<? if (!empty($this->Session->read('creacionMasivaUbicaciones.options'))) : ?>
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title">Listando <?= count($this->Session->read('creacionMasivaUbicaciones.options')); ?> itemes</h3>
				</div>
				<div class="panel-body">
					...
					<div class="table-responsive" style="max-height: 350px; overflow-y: auto;">
						<table class="table table-bordered" >
							...
							<thead>
								<? foreach ($this->Session->read('creacionMasivaUbicaciones.options') as $i => $cabecera) : ?>
								<th>
							...
							<tbody class="js-body">
							<? foreach ($this->Session->read('creacionMasivaUbicaciones.data') as $id => $valor) : if ($id == 1) continue; ?>
							<tr>
								<? foreach ($this->Session->read('creacionMasivaUbicaciones.options') as $i => $op) : ?>
