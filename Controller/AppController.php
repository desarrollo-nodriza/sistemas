...
class AppController extends Controller
{
	...
	*/
	 ...
	 * @param 		$formName 		String 		Nombre del formulario o indice de la petición
     * @return 		void
     */
    public function filtro($controlador = '', $accion = '', $formName = '')
    {
    	$redirect = array(
		...
		foreach ($this->request->data[$formName] as $campo => $valor) {
			if (!empty($valor)) {
	...
	public function sort_array_by_key(&$arr, $indice, $sort = SORT_DESC)
	{
		$auxIndice = array();
		...
		foreach ($arr as $im => $a) 
		{
			$auxIndice[$im] = $a[$indice];
		}

		# Ordenamos por indice
		if ($auxIndice)
		{
			array_multisort($auxIndice, $sort, $arr);
		}
	}
