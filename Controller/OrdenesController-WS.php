...
class OrdenesController extends AppController
{
	...
	public function admin_generar($id_orden = '')
	{
		...
		{	
			...
			if ($this->request->data['Dte']['Transporte'] > 0) {
			...
			if ($this->request->data['Dte']['tipo_documento'] == 39) {
				...
				if (isset($this->request->data['DscRcgGlobal']['ValorDR']) && $this->request->data['DscRcgGlobal']['ValorDR'] > 0) {
					...
					unset($this->request->data['DscRcgGlobal']);
				}
			}
			...
			if($this->Orden->Dte->saveAll($this->request->data)) {
				...
				if (!empty($id_dte)) {
					$this->redirect(array('controller' => 'ordenes', 'action' => 'editar', $id_dte['Dte']['id']));
				}
				...
				$this->redirect(array('controller' => 'ventas', 'action' => 'orden', $id_orden));
			}
			...
			if (empty($venta['Marketplace']['id'])) {
				...
				$opt['display'] = '[id]';
				$opt['filter[id_order]'] = '[' .$venta['Venta']['id_externo']. ']';
				...
				if ($venta['Marketplace']['marketplace_tipo_id'] == 1) {
					...
					$response = $ConexionLinio->call(
					    (new GenericRequest(
					        ...
					        'GetOrderComments',
					        GenericRequest::V1,
					        ['OrderId' => $venta['Venta']['id_externo']]
					    ))
					);
				...
				*/
				...
				if ($venta['Marketplace']['marketplace_tipo_id'] == 2) {
					...
					$VentaController->Mercadolibre = new Meli($venta['Marketplace']['api_user'], $venta['Marketplace']['api_key'], $venta['Marketplace']['access_token'], $venta['Marketplace']['refresh_token']);

					$mensajes = $VentaController->mercadolibre_obtener_mensajes($venta['Marketplace']['access_token'], $venta['Venta']['id']);
		...
		BreadcrumbComponent::add('órdenes de compra', '/ventas');
		BreadcrumbComponent::add('Ver Dte´s', '/ventas/view/'.$id_orden);
		BreadcrumbComponent::add('Generar Dte ');
		
		$this->set(compact('venta', 'comunas', 'tipoDocumento', 'traslados', 'dteEmitidos', 'codigoReferencia', 'medioDePago'));

	}

	public function admin_editar($id_dte = '')
	{
		$this->verificarTienda();
		...
		$venta = $this->request->data = $this->Orden->Dte->find(
			'first',
			array(
				'conditions' => array(
					'Dte.id' => $id_dte
				...
				'contain' => array(
					...
					'DteDetalle' => array(
						'fields' => array(
							'DteDetalle.id', 'DteDetalle.VlrCodigo', 'DteDetalle.NmbItem', 'DteDetalle.PrcItem', 'DteDetalle.QtyItem'
						)
					),
					'Venta' => array(
						'VentaDetalle' => array(
		...
		//carga de mensajes de la venta	
		$venta['Venta']['VentaMensaje'] = array();
		
		//----------------------------------------------------------------------------------------------------
		//carga de mensajes de prestashop
		if (empty($venta['Venta']['Marketplace']['id'])) {

			$ConexionPrestashop = new PrestaShopWebservice($venta['Venta']['Tienda']['apiurl_prestashop'], $venta['Venta']['Tienda']['apikey_prestashop'], false);
			...
			//----------------------------------------------------------------------------------------------------
			//carga de mensajes de Linio
			if ($venta['Venta']['Marketplace']['marketplace_tipo_id'] == 1) {

				$ConexionLinio = Client::create(new Configuration($venta['Venta']['Marketplace']['api_host'], $venta['Venta']['Marketplace']['api_user'], $venta['Venta']['Marketplace']['api_key']));
			...
			*/
			...
			//----------------------------------------------------------------------------------------------------
			//carga de mensajes de mercado libre
			if ($venta['Venta']['Marketplace']['marketplace_tipo_id'] == 2) {

				$VentaController = new VentasController();

				$VentaController->Mercadolibre = new Meli($venta['Venta']['Marketplace']['api_user'], $venta['Venta']['Marketplace']['api_key'], $venta['Venta']['Marketplace']['access_token'], $venta['Venta']['Marketplace']['refresh_token']);

				$mensajes = $VentaController->mercadolibre_obtener_mensajes($venta['Venta']['Marketplace']['access_token'], $venta['Venta']['Venta']['id']);
		...
		if (empty($this->request->data['Dte']['pdf']) && $this->request->data['Dte']['estado'] == 'dte_real_emitido' ) {
			...
			try {
				$this->generarPDFDteEmitido($venta['Venta']['id'], $id_dte, $this->request->data['Dte']['tipo_documento'], $this->request->data['Dte']['folio'], $this->request->data['Dte']['emisor'] );
			} catch (Exception $e) {
				if($e->getCode() < 300) {
					$this->Session->setFlash($e->getMessage() , null, array(), 'success');
				}
		...
		BreadcrumbComponent::add('Ordenes de compra', '/ventas');
		BreadcrumbComponent::add('Ver Dte´s', '/ventas/view/'.$this->request->data['Dte']['venta_id']);
		BreadcrumbComponent::add('Editar Dte ');
		
		$this->set(compact('venta', 'comunas', 'tipoDocumento', 'traslados', 'dteEmitidos', 'codigoReferencia', 'medioDePago', 'estadoSii'));
	}
	...
	{	
		...
		if ($this->request->is('post')
			...
			if ($enviar['status']['code'] == 200) {
				...
				$this->redirect(array('controller' => 'ventas', 'action' => 'view', $this->request->data['Orden']['id_orden']));
			}	
		}else{
